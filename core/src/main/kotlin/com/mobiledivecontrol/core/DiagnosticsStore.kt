...
class DiagnosticsStore {
    ...
    fun exportBundle(state: AppState): Map<String, String> {
        ...
        return mapOf(
            ...
            "pressure-sensors.json" to jsonObject(
                "waterCharacteristic" to "1625",
                "waterPressureKpa" to state.safety.waterPressureKpa,
                "waterRawHex" to state.waterPressureTelemetry?.rawHex,
                "waterReceivedAtEpochMs" to state.waterPressureTelemetry?.receivedAtEpochMs,
                "waterPacketCount" to state.waterPressureTelemetry?.packetCount,
                "waterSource" to state.waterPressureTelemetry?.source?.name,
                "waterAgeMs" to state.waterPressureTelemetry?.ageMs(pressureMonotonicMs()),
                "barometricCharacteristic" to "1627",
                "barometricPressureKpa" to state.safety.barometricPressureKpa,
                "barometricRawHex" to state.barometricPressureTelemetry?.rawHex,
                "barometricReceivedAtEpochMs" to state.barometricPressureTelemetry?.receivedAtEpochMs,
                "barometricPacketCount" to state.barometricPressureTelemetry?.packetCount,
                "barometricSource" to state.barometricPressureTelemetry?.source?.name,
                "surfaceBaselineKpa" to STANDARD_SURFACE_PRESSURE_KPA,
                "vacuumSurfaceReferenceKpa" to state.safety.surfaceAmbientKpa,
            ),
            "raw-packets.jsonl" to rawPackets.toList().joinToString(separator = "\n") { record ->
                jsonObject(
                    "timestamp" to record.timestamp.toString(),
                    "characteristic" to record.characteristic,
                    "payloadHex" to record.payloadHex,
                )
            },
